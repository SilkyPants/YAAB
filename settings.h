...
struct CycleValues // 7 bytes?
{
    unsigned char flags;                    // Flags using in marker cycle (CycleFlags)
    unsigned char markerState;              // Marker current state
    unsigned char eyesState;                // Eyes current state
    unsigned char shotsToGo;                // Shots to fire in 'cycle' (Burst)
    unsigned char debounceCharge;           // Counter used to detect trigger state changes
